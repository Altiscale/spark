...
export SPARK_VERSION=${SPARK_VERSION:-"2.4.4"}
...
if [ "x${DEBUG_MAVEN}" = "xtrue" ] ; then
  mvn_cmd="mvn -U -X $hadoop_profile_str -Phive-thriftserver -Phadoop-provided -Phive-provided -Psparkr -Pyarn -Pkinesis-asl -DskipTests install --log-file mvnbuild_20190128.log"
else
  mvn_cmd="mvn -U $hadoop_profile_str -Phive-thriftserver -Phadoop-provided -Phive-provided -Psparkr -Pyarn -Pkinesis-asl -DskipTests install --log-file mvnbuild_20190128.log"
fi
...
# AE-1369
echo "ok - start packaging a sparkr.zip for YARN distributed cache, this assumes user isn't going to customize this file"
pushd R/lib/
...
cp -rp $curr_dir/core/target/*.jar ./core/target/
cp -rp $curr_dir/sql/catalyst/target/*.jar ./sql/catalyst/target/
cp -rp $curr_dir/sql/core/target/*.jar ./sql/core/target/
cp -rp $curr_dir/launcher/target/*.jar ./launcher/target/
cp -rp $curr_dir/common/unsafe/target/*.jar ./common/unsafe/target/
cp -rp $curr_dir/common/tags/target/*.jar ./common/tags/target/
cp -rp $curr_dir/common/sketch/target/*.jar ./common/sketch/target/
cp -rp $curr_dir/resource-managers/yarn/target/*.jar ./resource-managers/yarn/target/
popd
...
cp -rp $curr_dir/external/kinesis-asl/target/*.jar ./external/kinesis-asl/target/
cp -rp $curr_dir/external/kinesis-asl-assembly/target/*.jar ./external/kinesis-asl-assembly/target/
popd
